...
import { MessageCircle, X, Send, Sparkles } from "@/components/icons"
...
export function AriaHelpChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "assistant",
      content: "Hi! I'm ARIA's help assistant. Ask me anything about how to use the platform!",
    },
  ])
  ...
  const handleSend = async (question?: string) => {
    ...
    try {
      ...
      // Use RAG context to generate response
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content:
            aiData.answer ||
            "I couldn't find specific information about that. Try rephrasing your question or contact support.",
        },
      ])
    } catch (error) {
      setMessages((prev) => [
  ...
  return (
    <>
      ...
      {!isOpen && messages.length === 1 && (
        <span className="fixed bottom-[4.5rem] right-6 h-3 w-3 bg-red-500 rounded-full z-50 animate-pulse" />
      )}
      ...
      {isOpen && (
        <Card className="fixed bottom-24 right-6 w-[380px] h-[500px] shadow-2xl z-50 flex flex-col border-0 overflow-hidden">
          {/* Header */}
          <CardHeader className="bg-gradient-to-r from-teal-500 to-teal-600 text-white py-4 px-4">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 bg-white/20 rounded-full flex items-center justify-center">
                <Sparkles className="h-5 w-5" />
              </div>
              <div>
                <h3 className="font-semibold">ARIA Help</h3>
                <p className="text-xs text-teal-100">Ask me anything about ARIA</p>
              </div>
            </div>
          </CardHeader>
          ...
          <div className="p-3 bg-white border-t">
            ...
            <p className="text-xs text-gray-400 mt-2 text-center">Press F1 or Cmd+/ to open help</p>
          </div>
        </Card>
      )}
    </>
